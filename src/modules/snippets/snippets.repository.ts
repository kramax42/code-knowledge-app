...
export class SnippetsRepository {
  ...
  async create(dto: CreateSnippetDto) {
    ...
    try {
      ...
      const categoryDoc =
        await this.categoriesRepository.incrementSnippetsAmount(dto.category);
  ...
  async deleteById(id: string) {
    ...
    try {
      ...
      const categoryDoc =
        await this.categoriesRepository.decrementSnippetsAmount(
          snippet.category,
        );
  ...
  async updateById(id: string, dto: UpdateSnippetDto) {
    ...
    try {
      ...
      if (existedSnippet.category != dto.category) {
        ...
        categoryDoc = await this.categoriesRepository.incrementSnippetsAmount(
          dto.category,
        );
        if (!categoryDoc) {
          await session.abortTransaction();
