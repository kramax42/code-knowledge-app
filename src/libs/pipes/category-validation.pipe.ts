...
export class CategoryValidationPipe implements PipeTransform {
  ...
  async transform(value: string, metadata: ArgumentMetadata) {
    ...
    const objVal = Object.values(categories).find(
      (c) => c.categoryURLName == value,
    );
    if (!objVal) {
      throw new BadRequestException(CATEGORY_VALIDATION_ERROR);
    }
    const objKey = Object.keys(categories).find(
      (key) => categories[key].categoryURLName === objVal.categoryURLName,
    );
